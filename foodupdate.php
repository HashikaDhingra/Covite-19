...
input[type=text]:focus, input[type=password]:focus {
  ...
  error_reporting(0);
  session_start();
  $db=mysqli_connect("localhost","root","","covite-19") or die(mysqli_error());
  
  $name = $_POST['account'];

$query=mysqli_query($db,"SELECT * FROM fregister where account='$account'")or die(mysqli_error());
...
<hr>
      ...
      $query = "UPDATE `fregister` SET `name`='$name',`email`='$email',`phone`='$phone',`address`='$address',`account`='$account',`password`='$password',`confirmpsw`='$confirmpsw',`time`='$time',`info`='$info' WHERE account='$account'";
