  ...
  <header>
    <div Class="topnav">
      ...
      <div class="login-container">
        <form action="quratinelogin.php" method="POST">
    ...
    <form action="quarantineregister.php" method="POST">
      <div class = "vertical"></div>
    <div class="container">
      <h1>Register as a Room Provider.</h1>
      <p>Please fill in this form to create an account.</p>
      <hr>
      <label for="email"><b>Name* </b></label>
      <input type="text" placeholder="Enter name" name="firm" id="Name" required>
...
<br><br>
  ...
  <input type="submit" style="height:50px; width:100px; background-color:#05164C;color:white;border:none;">
      ...
      <input type="password" placeholder="Repeat Password" name="confirmpsw" id="psw-repeat" required>
       ...
       <br><br><br>
       <label for="timings"><b>Available Beds*</b></label>
       <input type="text" placeholder="Beds Availability" name="available" id="aname" required>
  ...
  <div style="margin-top: -1380px;    margin-left: 115px;">
  <?php 
          $con=mysqli_connect("localhost","root","","covite-19") or die(mysqli_error());
          $sql=mysqli_query($con,"SELECT `name`, `email`, `phone`, `address`,`account`, `available`, `info` FROM qregister");

          if (mysqli_num_rows($sql) > 0);
          {
            while ($row = mysqli_fetch_assoc($sql)) {
              echo "<div class='doctor-container' style='margin-top:10px'>
                      <div class='fontstyle'>
                        <img src='verified.png' align='right'>
                        <b>
                      <h3>{$row['account']}</h3>
                     <p align='left'>{$row['name']}<br><a href='mailto:{$row['email']}'>{$row['email']}</a>
                      <br>{$row['address']}<br>Available:{$row['available']}<br>
                      <br>{$row['info']}<br>
